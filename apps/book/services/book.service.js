	...
	return storageService.query(BOOKS_KEY).then(books => {
		if (filterBy.title) {
			...
			books = books.filter(book => {
				return regex.test(book.title) && book.listPrice.amount >= filterBy.price
			})
		}
		if (filterBy.price) {
...
function addGoogleBook(googleBook) {
    const { title, description, authors, publishedDate, categories, pageCount, language, imageLinks } = googleBook.volumeInfo
    ...
    const book = {
        ...
        language,
        listPrice: {
            amount: utilService.getRandomIntInclusive(30, 260),
            currencyCode: 'USD',
